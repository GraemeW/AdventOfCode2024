...
void Rope::AddToRopePlus(long long nextRopeBit)
{
    std::vector<RopeNode*> newLeafNodes;
    if (leafNodes.size()==0) { leafNodes.push_back(root); }
    ...
    for (auto childRopeNode : leafNodes)
    {
        ...
        string concatenateOutputString = std::to_string((childRopeNode->ropeOutput)) + std::to_string(nextRopeBit);
        std::stringstream concatenateOutputConvert(concatenateOutputString);
        long long concatenateOutput;
        concatenateOutputConvert >> concatenateOutput;
        
        root->addChild = new RopeNode(addOutput);
        root->multiplyChild = new RopeNode(multiplyOutput);
        root->concatenateChild = new RopeNode(concatenateOutput);
        
        newLeafNodes.push_back(root->addChild);
        newLeafNodes.push_back(root->multiplyChild);
        newLeafNodes.push_back(root->concatenateChild);
    }
